...
fn build_aux_col_p1<E: FieldElement<BaseField = Felt>>(
    ...
    // keep track into of the index into the list of inserted rows; we can use this index because
    // the sequence in which items are pushed into the overflow table is exactly the same as the
    // sequence in which the rows are added to the list of rows.
    let mut inserted_row_idx = 0;
    ...
    for (clk, update) in aux_trace_hints.overflow_table_hints() {
        ...
        match update {
            OverflowTableUpdate::RowInserted => {
                result[result_idx] = result[clk] * row_values[inserted_row_idx];
                inserted_row_idx += 1;
            }
            OverflowTableUpdate::RowRemoved(removed_row_idx) => {
                result[result_idx] = result[clk] * inv_row_values[*removed_row_idx];
            }
        }
    }
